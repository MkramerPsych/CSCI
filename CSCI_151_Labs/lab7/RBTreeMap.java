...
implements MapADT<K,V> {
	...
	if(isEmpty()) { //if not contained in map
		return null;
	}
	else {
		int c = searchKey.compareTo(key);
		if(c < 0) {
    ...
    protected V put(K key, V value, RBTreeMap<K,V> parent, RBTreeMap<K,V> gran){
       ...
       if(isEmpty()){   // base case I:  empty tree
          ...
          int comp = key.compareTo(this.key); //make a comparison variable
          if(comp==0){  // base case II:  key is already present in root
        	  this.value = value;
        	  rvalue = value;
          }
       ...
       if(parent==null){ // adjustment case I:  this is the root, so paint it black
    	   this.color = BLACK;
       }
       else if(color==RED && parent.color==RED){// consecutive red nodes -- do something!
	   RBTreeMap<K,V> uncle = null; // first, find the uncle
	   if(parent.key.compareTo(gran.key)<0)
	       uncle = gran.right;
	   else
	       uncle = gran.left;
	   
	   if(uncle.color==RED){  // adjustment case II:  the uncle is red, so recolor
		   parent.color = uncle.color = BLACK;
		   gran.color = RED;
	   }
	   else {// adjustment case II:  the uncle is black, so rotate
		   int c1 = this.key.compareTo(parent.key);
		   int c2 = parent.key.compareTo(gran.key);
	       if(c1 < 0 && c2 < 0) { //LL case
	    	   rotateRight(gran);
	       }
	       else if (c1 > 0 && c2 < 0) { //LR case
	    	   rotateLeft(parent);
	    	   rotateRight(gran);
	       }
	       else if (c1 < 0 && c2 > 0) { //RL case
	    	   rotateRight(parent);
	    	   rotateLeft(gran);
	       }
	       else if (c1 > 0 && c2 > 0) { //RR case
	    	   rotateLeft(gran);
	       }
